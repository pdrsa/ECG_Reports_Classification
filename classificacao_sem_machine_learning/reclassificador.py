...
dicionario = pd.read_csv("../light_data/DicionarioECG_semSiglas.csv", index_col = 0)
...
with open('../../data/resultados/scorings2.csv', 'w') as f:
    f.write("id_exame, scorings\n")
errors = []
for i in range(0, len(db), batch):
    print(i,"/",len(db))
    try:
        scores = [[max([partial_ratio(clean_text(text), clean_text(diag)) for diag in row[2:] if (clean_text(diag) != '')]) for row in dicionario.itertuples()]          for text in db["CONTEUDO"][i:i+batch]]
        with open('../../data/resultados/scorings2.csv', 'a') as f:
