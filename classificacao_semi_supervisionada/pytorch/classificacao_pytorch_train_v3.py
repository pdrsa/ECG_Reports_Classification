...
def tokenize(text, tokenizer, fit = False):
    ...
    # Saving tokenizer
    with open("../../light_data/pytorch_tokenizer_"+str(TOP_WORDS)+".pickle", 'wb') as handle:
...
if REBUILD_DATA:
    ...
    np.save("../../../data/training_data/training_data_X_"+str(TOP_WORDS)+".npy", train_X)
else:
    labels       = np.load('../../../data/resultados/scores/allLabels.npy')
    greenZoneIdx = np.load('../../light_data/greenZoneIndex.npy')
    labels       = [labels[i] for i in greenZoneIdx]
    train_X      = np.load('../../../data/training_data/training_data_X_'+str(TOP_WORDS)+'.npy', allow_pickle = True)
...
print("\nSaving model...")
torch.save(net.state_dict(), "../../../data/trained_models/pytorch_checkpoint_4.pth")
print("Finish!")
